<template>
  <div class="settings-tab">
    ...
    <div class="body">
      ...
      <template v-if="selectedTab?.tabName == TAB_NOT_COMPONETS">
        <DebugPanel :data="data" />
      </template>
      <template v-if="selectedTab?.tabName == TAB_RULE_SETTINGS">
        <RulePanel :data="data" />
      </template>
    </div>
  </div>
...
import DebugPanel from "./settings/DebugPanel.vue";
import RulePanel from "./settings/RulePanel.vue";
  ...
  data() {
    return {
      ...
      settingsTabs: [
        ...
        { tabName: TABS.TAB_NOT_COMPONETS, title: "нЕкомпоненты" },
